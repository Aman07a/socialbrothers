 <?php
    ...
    // Get the post slug
    if ($current_post) {
        $current_page_slug = $current_post->post_name;
    } else {
        $current_page_slug = $current_page_id;
    }

    $default__category__event = isset($_GET['events']) ? sanitize_text_field($_GET['events']) : 'utrecht';
    ?>

 <form action="<?php echo esc_url(home_url($current_page_slug)); ?>" method="get" class="filter">
